...
def generate_product_prices(all_products):
    """Generate stable fallback prices for products without a selected SKU."""
    product_prices = dict()
...
def init_search_engine(all_products, num_products=None, product_data_path=None):
    """Open the deterministic BM25 index, rebuilding a stale cache if needed."""
    if num_products == 100:
        index_name = 'products_100.sqlite3'
    elif num_products == 1000:
        index_name = 'products_1k.sqlite3'
    elif num_products == 100000:
        index_name = 'products_100k.sqlite3'
    elif num_products is None:
        index_name = 'products.sqlite3'
    else:
        raise NotImplementedError(f'num_products being {num_products} is not supported yet.')

    index_path = os.path.join(BASE_DIR, '../search_engine', index_name)
    product_sha256 = (
        sha256_file(product_data_path)
        if product_data_path is not None
        else product_data_fingerprint(all_products)
    )
    try:
        return MultiFieldBM25Searcher(
            index_path,
            expected_product_sha256=product_sha256,
        )
    except SearchIndexError:
        build_index(
            all_products,
            index_path,
            product_data_sha256=product_sha256,
        )
        return MultiFieldBM25Searcher(
            index_path,
            expected_product_sha256=product_sha256,
