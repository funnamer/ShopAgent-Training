...
**Serving the local Qwen3-4B model with vLLM:**

```bash
cd single_eval

# Original Qwen3-4B model
python serve_vllm.py

# The same base model with a selected LoRA checkpoint
python serve_vllm.py checkpoint-100

# In another terminal, run the evaluation against either deployment
python agent.py --yaml_name configs/standard/qwen3_4b_local.yaml
```

The API model name remains `qwen3-4b-action-only` in both modes. The launcher
also accepts an absolute checkpoint path and supports `--dry-run` to validate
and display the generated vLLM command without loading the model. It serves a
32,768-token context window by default; use `--max-model-len` to override it.
